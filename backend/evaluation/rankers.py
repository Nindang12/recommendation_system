...
class EvaluationRankers:
    ...
    async def rank(
        ...
        *,
        ...
        heldout_relevant_ids: Optional[List[str]] = None,
    ) -> Tuple[List[Dict[str, Any]], float]:
        started = time.perf_counter()
        method = str(method).lower()
        with self._allow_heldout_targets(heldout_relevant_ids or []):
            if method == "random":
    ...
    @contextmanager
    def _allow_heldout_targets(self, heldout_ids: List[str]):
        """Evaluation-only hook for leave-one-edge-out reconstruction.

        Production PGPR excludes already connected targets. In reconstruction
        evaluation the selected existing edge is treated as held out, so its
        target must not be removed by that exclude list.
        """
        heldout = {str(x) for x in heldout_ids if x}
        recommender = getattr(self.rs, "recommender", None)
        graph_repo = getattr(recommender, "graph_repo", None)
        original = getattr(graph_repo, "get_exclude_target_ids", None)
        if not heldout or original is None:
            yield
            return

        def patched(source_id: str, source_type: str, target_type: str) -> List[str]:
            return [str(x) for x in original(source_id, source_type, target_type) if str(x) not in heldout]

        graph_repo.get_exclude_target_ids = patched
        try:
            yield
        finally:
            graph_repo.get_exclude_target_ids = original
