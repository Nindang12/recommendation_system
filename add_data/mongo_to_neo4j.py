...
from skill_identity_utils import normalize_skill_dict, resolve_skill_record
...
def skill_ref(skill_obj: Any) -> Optional[Dict[str, Any]]:
    if isinstance(skill_obj, dict):
        normalized = normalize_skill_dict(skill_obj)
        if not normalized:
            return None
        return {
    ...
    if isinstance(skill_obj, str) and skill_obj.strip():
        resolved = resolve_skill_record(skill_obj)
        if not resolved.get("skill_id"):
            return None
        return {
    ...
    for idx, ex in enumerate(experts, 1):
        ...
        try:
            ...
            for sm in skills_methods:
                ref = skill_ref(sm if isinstance(sm, dict) else {"name": sm})
                if not ref:
                    continue
                session.run(
                    """
            ...
            for interest in safe_get(ex, "research_capacity", "research_interests") or []:
                topic_name = str(interest.get("name") if isinstance(interest, dict) else interest).strip()
                if not topic_name:
                    continue
                session.run(
                    "MERGE (t:ResearchTopic {name: $name}) SET t.updated_at = datetime()",
...
def sync_enterprises_new(session, direction_ids: set, topic_ids: set):
    ...
    for idx, ent in enumerate(enterprises, 1):
        ...
        try:
            ...
            # technology_needs.required_skills (MERGE by skill_id)
            tech_needs = rd_profile.get("technology_needs") or []
            for need in tech_needs:
                if not isinstance(need, dict):
                    continue
                for skill in need.get("required_skills") or []:
                    ref = skill_ref(skill if isinstance(skill, dict) else {"name": skill})
                    if not ref:
                        continue
                    session.run(
                        """
                    ...
                    session.run(
                        ...
                        enterprise_id=enterprise_id,
                        skill_id=ref["skill_id"],
                        level=skill.get("proficiency_level") if isinstance(skill, dict) else None,
...
def sync_projects_new(session, direction_ids: set, topic_ids: set):
    ...
    for idx, pr in enumerate(projects, 1):
        ...
        try:
            ...
            # required_skills (MERGE by skill_id)
            for skill in req.get("required_skills") or []:
                ref = skill_ref(skill if isinstance(skill, dict) else {"name": skill})
                if not ref:
                    continue
...
def build_expert_name_index():
    from expert_identity_utils import (
        build_expert_name_index as _build_name_index,
        build_orcid_index,
        expert_display_name,
    )
    ...
    experts = list(
        mdb.experts.find(
            {
                "kg_sync_status": {"$ne": "disabled"},
                "$or": [
                    {"merged_into": {"$exists": False}},
                    {"merged_into": None},
                ],
            },
            {"expert_id": 1, "basic_info.name": 1, "expert_alias_ids": 1, "identifiers": 1},
        )
    )
    index = _build_name_index(experts)
    for eids in build_orcid_index(experts).values():
        if len(eids) < 2:
            continue
        union = sorted(set(eids))
        for ex in experts:
            expert_id = ex.get("expert_id")
            if expert_id not in union:
                continue
            key = normalize_author_name(expert_display_name(ex))
            if not key:
                continue
...
def sync_paper_coauthor_edges(session):
    """Connect scientific papers to matching Expert nodes and create Expert CO_AUTHORED edges."""
    import time

    logger.info("Syncing paper co-author relationships...")
    expert_name_index = build_expert_name_index()
    product_query = {"metadata.authors.0": {"$exists": True}}
    product_projection = {
        "product_id": 1,
        "name": 1,
        "year": 1,
    ...
    def _run_with_retry(query, **params):
        for attempt in range(6):
            try:
                ...
                err = str(exc)
                if "DeadlockDetected" not in err and "CursorNotFound" not in err:
                    raise
                if attempt == 5:
                    raise
                time.sleep(0.2 * (2 ** attempt))

    def _upsert_coauthor_pairs(rows):
        """rows: list of {left_id, right_id, product_ids}"""
        nonlocal coauthor_pair_ops
        if not rows:
            return
        _run_with_retry(
            """
            UNWIND $rows AS row
            MATCH (a:Expert {expert_id: row.left_id})
            MATCH (b:Expert {expert_id: row.right_id})
            MERGE (a)-[r:CO_AUTHORED]-(b)
            WITH r, row, coalesce(r.product_ids, []) AS existing
            SET r.product_ids = existing + [x IN row.product_ids WHERE NOT x IN existing | x],
                r.paper_count = size(r.product_ids),
                r.updated_at = datetime()
            """,
            rows=rows,
        )
        coauthor_pair_ops += len(rows)

    def flush_develop_rows():
        nonlocal develops_edges, develop_rows
        if not develop_rows:
            return
        _run_with_retry(
            """
            UNWIND $rows AS row
            MATCH (e:Expert {expert_id: row.expert_id})
            MATCH (o:Product {product_id: row.product_id})
            MERGE (e)-[r:DEVELOPS]->(o)
            SET r.source = coalesce(r.source, 'paper_author_match')
            """,
            rows=develop_rows,
        )
        develops_edges += len(develop_rows)
        develop_rows = []

    def flush_coauthor_pending(batch_size=50):
        if not coauthor_pending:
            return
        keys = sorted(coauthor_pending.keys())[:batch_size]
        rows = []
        for key in keys:
            product_ids = sorted(coauthor_pending.pop(key, set()))
            if not product_ids:
                continue
        ...
        _upsert_coauthor_pairs(rows)

    # Phase 1: scan Mongo in pages (no Neo4j co-author writes during scan).
    page_size = 2000
    last_id = None
    while True:
        page_query = dict(product_query)
        if last_id is not None:
            page_query["_id"] = {"$gt": last_id}
        products = list(
            mdb.products.find(page_query, product_projection)
            .sort("_id", 1)
            .limit(page_size)
        )
        if not products:
        ...
        for product in products:
            ...
            matched_ids = set(product.get("developer_ids") or product.get("developed_by") or [])
            for author in _paper_authors(product):
                for expert_id in expert_name_index.get(normalize_author_name(author), []):
                    matched_ids.add(expert_id)
            ...
            for expert_id in matched_ids:
                develop_rows.append({"expert_id": expert_id, "product_id": product_id})

            for i, a_id in enumerate(matched_ids):
                for b_id in matched_ids[i + 1:]:
                    coauthor_pending.setdefault(_canonical_pair(a_id, b_id), set()).add(product_id)

            if len(develop_rows) >= 500:
                flush_develop_rows()

        flush_develop_rows()
        if scanned_products % 10000 == 0:
            logger.info(
                "Paper co-author scan progress: scanned=%s matched=%s pending_pairs=%s develops_ops=%s",
                scanned_products,
                matched_product_count,
                len(coauthor_pending),
                develops_edges,
            )

    logger.info(
        "Mongo scan done: scanned=%s matched=%s unique_coauthor_pairs=%s; writing Neo4j CO_AUTHORED...",
        scanned_products,
        matched_product_count,
        len(coauthor_pending),
    )

    # Phase 2: flush co-author pairs to Neo4j (Mongo cursor already closed).
    while coauthor_pending:
        flush_coauthor_pending(batch_size=50)
        if coauthor_pair_ops and coauthor_pair_ops % 500 == 0:
            logger.info(
                "Paper co-author write progress: pairs_written=%s remaining=%s",
                coauthor_pair_ops,
                len(coauthor_pending),
            )

    logger.info(
        "Paper co-author sync done: matched_products=%s develops_ops=%s coauthor_pairs=%s",
        matched_product_count,
        develops_edges,
        coauthor_pair_ops,
    )
...
def sync_experts(session):
    ...
    for idx, ex in enumerate(experts, 1):
        ...
        try:
            ...
            # Expert -> Skill (HAS_SKILL) with proficiency level
            skills_methods = safe_get(ex, "research_capacity", "skills_methods") or []
            for sm in skills_methods:
                ref = skill_ref(sm if isinstance(sm, dict) else {"name": sm})
                if not ref:
...
# MAIN FUNCTION
# ==========================================
def main(clear_graph: bool = False, skip_verify: bool = False, skip_indexes: bool = False, coauthors_only: bool = False):
    """
    ...
    try:
        with driver.session() as session:
            ...
            if coauthors_only:
                logger.info("Running targeted co-author / paper-author sync only.")
                sync_paper_coauthor_edges(session)
                if not skip_verify:
                    verify_sync(session)
                return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--coauthors-only",
        action="store_true",
        help="Chỉ sync DEVELOPS/CO_AUTHORED từ paper authors (nhanh, sau merge identity).",
    )
    ...
    main(
        ...
        coauthors_only=args.coauthors_only,
    )
