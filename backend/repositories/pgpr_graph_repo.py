...
class PGPRGraphRepository:
    ...
    def upsert_skill_relationships(self, entity_type: str, entity_id: str, skills: List[Any]) -> None:
        label = self._safe_label(entity_type)
        id_prop = self._id_prop(label)
        relations = {
            "Expert": "HAS_SKILL",
        ...
        for skill in skills or []:
            if isinstance(skill, dict):
                raw_name = skill.get("name") or skill.get("skill") or skill.get("technology")
                ...
                skill_id = ""
                category = None
            if skill_id and raw_name:
                skill_name = str(raw_name).strip()
            elif skill_id:
                skill_name = skill_id.replace("_", " ").removeprefix("SKILL ").title()
            elif raw_name:
                resolved = resolve_skill_record(raw_name, category=category)
                skill_id = str(resolved.get("skill_id") or "").strip()
                skill_name = str(resolved.get("name") or raw_name).strip()
            else:
                continue
            if not skill_id:
                continue
        ...
        try:
            ...
            topic_rows = self.run_read(
                ...
                WHERE NOT (e1)-[:PARTICIPATES_IN]->(p)
                  AND NOT EXISTS {
                    MATCH (existing:Expert)-[:PARTICIPATES_IN]->(p)
                    WHERE toLower(coalesce(existing.name, "")) = toLower(coalesce(e1.name, ""))
                  }
                ...
                WHERE NOT (e2)-[:PARTICIPATES_IN]->(p)
                  AND NOT EXISTS {
                    MATCH (existing:Expert)-[:PARTICIPATES_IN]->(p)
                    WHERE toLower(coalesce(existing.name, "")) = toLower(coalesce(e2.name, ""))
                  }
                ...
                RETURN DISTINCT e.expert_id as expert_id,
                       ...
                       3 as min_hops,
                       "shared_topic" as candidate_source
                LIMIT $limit
                """,
            ...
            for record in topic_rows:
                ...
                if eid:
                    row["candidate_sources"] = [row.pop("candidate_source", "shared_topic")]
                    out[eid] = row

            funder_rows = self.run_read(
                """
                MATCH (p:Project {project_id: $project_id})<-[:FUNDS]-(f:Funder)-[:FUNDS]->(p2:Project)<-[:PARTICIPATES_IN]-(e:Expert)
                WHERE p2.project_id <> $project_id
                  AND NOT (e)-[:PARTICIPATES_IN]->(p)
                  AND NOT EXISTS {
                    MATCH (existing:Expert)-[:PARTICIPATES_IN]->(p)
                    WHERE toLower(coalesce(existing.name, "")) = toLower(coalesce(e.name, ""))
                  }
                OPTIONAL MATCH (e)-[:LOCATED_IN]->(l:Location)
                RETURN DISTINCT e.expert_id as expert_id,
                       e.name as name,
                       ...
                       e.citation_count as citations,
                       e.publication_count as publications,
                       count(DISTINCT p2) as matched_projects,
                       3 as min_hops
            ...
            for record in funder_rows:
                row = dict(record)
                eid = row.get("expert_id")
                if not eid:
                    continue
                if eid in out:
                    out[eid].setdefault("candidate_sources", []).append("shared_funder")
                    out[eid]["matched_projects"] = row.get("matched_projects")
                else:
                    row["candidate_sources"] = ["shared_funder"]
                    out[eid] = row

            collaborator_rows = self.run_read(
                """
                MATCH (p:Project {project_id: $project_id})<-[:PARTICIPATES_IN]-(seed:Expert)-[:PARTICIPATES_IN]->(p2:Project)<-[:PARTICIPATES_IN]-(e:Expert)
                WHERE p2.project_id <> $project_id
                  AND e.expert_id <> seed.expert_id
                  AND NOT (e)-[:PARTICIPATES_IN]->(p)
                  AND NOT EXISTS {
                    MATCH (existing:Expert)-[:PARTICIPATES_IN]->(p)
                    WHERE toLower(coalesce(existing.name, "")) = toLower(coalesce(e.name, ""))
            ...
            for record in collaborator_rows:
                row = dict(record)
                eid = row.get("expert_id")
                if not eid:
                    continue
                if eid in out:
                    out[eid].setdefault("candidate_sources", []).append("co_participation")
                    out[eid]["matched_projects"] = max(
                        int(out[eid].get("matched_projects") or 0),
                        int(row.get("matched_projects") or 0),
                    )
                else:
                    row["candidate_sources"] = ["co_participation"]
                    out[eid] = row
            ...
            if out:
                return sorted(
                    out.values(),
                    key=lambda item: (
                        int(item.get("min_hops") or max_depth),
                        -int(item.get("matched_projects") or 0),
                    ),
                )[:limit]
            return self.find_candidate_experts_generic(project_id, max_depth=max_depth, limit=limit)
    ...
    def find_candidate_experts_for_expert(self, expert_id: str,
        ...
        try:
            result3 = self.run_read(
                """
                MATCH (e:Expert {expert_id: $expert_id})-[r:CO_AUTHORED]-(e2:Expert)
                WHERE e2.expert_id <> $expert_id
                RETURN DISTINCT e2.expert_id AS expert_id,
                       e2.name AS name,
                       e2.location AS location,
                       ...
                       e2.citation_count AS citations,
                       e2.publication_count AS publications,
                       coalesce(r.paper_count, 1) AS shared_papers
            ...
            for record in result3:
                ...
                if eid in by_id:
                    ...
                    src.add("co_authored")
                    by_id[eid]["candidate_sources"] = sorted(src)
                    by_id[eid]["shared_papers"] = max(
                        int(by_id[eid].get("shared_papers") or 0),
                        int(d.get("shared_papers") or 0),
                    )
                else:
                    d["candidate_sources"] = ["co_authored"]
                    by_id[eid] = d
        except Exception as e:
            logger.debug("Candidate experts for expert (co-authored) query failed: %s", e)
        return list(by_id.values())
