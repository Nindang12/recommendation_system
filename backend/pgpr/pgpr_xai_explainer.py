...
_TECH_ID_PREFIX_RE = re.compile(r"^(EXP|PRJ|FUN|ENT|OA)_", re.IGNORECASE)


def _is_id_like(name: str) -> bool:
    """True when a string looks like an internal entity key, not a display name."""
    s = (name or "").strip()
    if not s:
        return False
    if _ID_LIKE_RE.match(s):
        return True
    if _TECH_ID_PREFIX_RE.match(s):
        return True
    if s.count("_") >= 2 and s.replace("_", "").replace(".", "").isalnum():
        return True
    return False


def _humanize_technical_id(raw: str) -> str:
    """Best-effort display label from IDs like OA_Alberto_Bardelli or EXP_0001."""
    s = (raw or "").strip()
    if not s:
        return s
    if not _is_id_like(s):
        return s
    for prefix in ("OA_", "EXP_", "PRJ_", "FUN_", "ENT_"):
...
def _name_from_entity_doc(doc: Optional[Dict[str, Any]]) -> str:
    if not isinstance(doc, dict):
        return ""
    for key in ("name", "title"):
        value = doc.get(key)
        if value and not _is_id_like(str(value)):
...
def _resolve_display_name(name: str, entity_id: str = "") -> str:
    """Prefer a real display name; otherwise humanize technical IDs."""
    candidate = (name or "").strip()
    eid = (entity_id or "").strip()
    if candidate and not _is_id_like(candidate) and candidate != eid:
        return candidate
    if eid:
        humanized = _humanize_technical_id(eid)
        if humanized and not _is_id_like(humanized):
            return humanized
    if candidate:
        humanized = _humanize_technical_id(candidate)
        if humanized and not _is_id_like(humanized):
            return humanized
    return candidate or eid or "Unknown"
...
def _enrich_source_context(
    ...
    ctx = dict(source_context)
    sid = str(ctx.get("source_id") or "").strip()
    current = str(ctx.get("source_name") or "").strip()

    entity_name = _name_from_entity_doc(ctx.get("source_entity"))
    if entity_name:
        ctx["source_name"] = entity_name
    elif not current or _is_id_like(current) or current == sid:
        ctx["source_name"] = _resolve_display_name(current, sid)
    return ctx
...
def _build_name_registry(
    ...
    ctx = _enrich_source_context(source_context) or {}

    sid = str(ctx.get("source_id") or "").strip()
    if sid:
        registry[sid] = _resolve_display_name(
            str(ctx.get("source_name") or ""), sid
        )

    rid = str(recommendation.get("id") or "").strip()
    rname = str(
        recommendation.get("name") or recommendation.get("title") or ""
    ).strip()
    if rid:
        registry[rid] = _resolve_display_name(rname, rid)
    if rname:
        registry[rname] = registry.get(rid) or _resolve_display_name(rname, rid)

    for path in recommendation.get("reasoning_paths") or []:
        if not isinstance(path, dict):
            continue
        entities = path.get("entities") or []
        entity_names = path.get("entity_names") or path.get("node_names") or []
        for idx, entity in enumerate(entities):
            if not isinstance(entity, dict):
                continue
            eid = str(entity.get("id") or "").strip()
            ename = str(entity.get("name") or "").strip()
            resolved = _resolve_display_name(ename, eid)
            if eid:
                registry[eid] = resolved
            if ename:
                registry[ename] = resolved
        for idx, raw_name in enumerate(entity_names):
            eid = ""
            if idx < len(entities) and isinstance(entities[idx], dict):
                eid = str(entities[idx].get("id") or "").strip()
            resolved = _resolve_display_name(str(raw_name), eid)
            registry[str(raw_name)] = resolved
            if eid:
                registry[eid] = resolved
    return registry


def _enrich_path_for_xai(
    path: Dict[str, Any],
    registry: Dict[str, str],
) -> Dict[str, Any]:
    enriched = dict(path)
    ...
    if entities:
        ...
        for entity in entities:
            ...
            item["name"] = registry.get(eid) or _resolve_display_name(ename, eid)
            new_entities.append(item)
        enriched["entities"] = new_entities

    for field in ("entity_names", "node_names"):
        names = enriched.get(field)
        if isinstance(names, list):
            enriched[field] = [
                registry.get(str(name))
                or _resolve_display_name(str(name), "")
                for name in names
            ]

    for field in ("path", "explanation"):
        value = enriched.get(field)
        if isinstance(value, str) and value.strip():
            enriched[field] = _sanitize_display_text(value, registry)
    return enriched


def _enrich_recommendation_for_xai(
    recommendation: Dict[str, Any],
    source_context: Optional[Dict[str, Any]],
) -> tuple[Dict[str, Any], Optional[Dict[str, Any]], Dict[str, str]]:
    rec = dict(recommendation)
    ctx = _enrich_source_context(source_context)
    registry = _build_name_registry(rec, ctx)

    rid = str(rec.get("id") or "").strip()
    rec["name"] = registry.get(rid) or _resolve_display_name(
        str(rec.get("name") or rec.get("title") or ""), rid
    )

    paths = rec.get("reasoning_paths") or []
    if paths:
        rec["reasoning_paths"] = [
            _enrich_path_for_xai(path, registry)
            if isinstance(path, dict)
            else path
            for path in paths
        ]
    return rec, ctx, registry


def _sanitize_display_text(text: str, registry: Dict[str, str]) -> str:
    ...
    for key in sorted(registry.keys(), key=len, reverse=True):
        ...
        out = re.sub(
            rf"\bdự án\s+chuyên gia\s+{re.escape(key)}\b",
            f"chuyên gia {value}",
            out,
            flags=re.IGNORECASE,
        )
        out = re.sub(
            rf"\bchuyên gia\s+{re.escape(key)}\b",
            value,
            out,
            flags=re.IGNORECASE,
        )
...
def _is_expert_to_expert(
    rec_type: str,
    source_context: Optional[Dict[str, Any]],
) -> bool:
    return (
        (rec_type or "").lower() == "expert"
        and (source_context or {}).get("source_type") == "Expert"
    )
...
def _source_ref_vi(source_context: Optional[Dict[str, Any]]) -> str:
    ...
    display = _resolve_display_name(sn, sid)
...
- Nếu có dự án/chuyên gia trung gian, nêu lịch sử tài trợ/cộng tác như một bằng chứng."""
                ...
                focus = f"""TRỌNG TÂM (Expert → Expert):
- Đây là gợi ý đối tác/cộng tác viên nghiên cứu giữa hai chuyên gia, KHÔNG phải gợi ý chuyên gia cho dự án.
- Nêu điểm chung về lĩnh vực, chủ đề nghiên cứu, kỹ năng hoặc mạng lưới cộng tác giữa {src} và {label_vi} **{recommendation_name}**.
- Nếu có dự án/quỹ/chuyên gia trung gian, nêu họ như cầu nối trong mạng lưới nghiên cứu."""
            else:
                focus = f"""TRỌNG TÂM:
...
def _format_paths_for_prompt(paths: List[Dict[str, Any]], language: str) -> str:
    ...
    for i, p in enumerate(paths, 1):
        sc = p.get("score", 0.0)
        node_names = p.get("node_names") or p.get("entity_names") or []
        if isinstance(node_names, list) and len(node_names) >= 2:
            arrow = " → " if language == "vi" else " -> "
            chain = arrow.join(
                _resolve_display_name(str(x), str(x)) for x in node_names
            )
...
def build_improved_prompt_from_paths(
    ...
    rules = f"""BẠN LÀ: Trợ lý AI chuyên nghiệp phân tích dữ liệu cho hệ thống gợi ý.

QUY TẮC BẮT BUỘC:
1. Viết 3–5 câu tiếng Việt thật tự nhiên, lưu loát, mang tính tư vấn chuyên nghiệp.
2. TUYỆT ĐỐI KHÔNG dùng các từ ngữ kỹ thuật như: "đường dẫn", "node", "edge", "đồ thị tri thức", "KG". Thay vào đó hãy dùng: "Hệ thống ghi nhận", "Dữ liệu cho thấy", "Kinh nghiệm thực tế", "Mạng lưới kết nối".
...
6. Cấu trúc lý tưởng: [Bối cảnh nguồn đúng loại thực thể] + [Phân tích điểm chung/kết nối] + [Lợi ích mang lại].
...
10. TUYỆT ĐỐI KHÔNG dùng mã định danh kỹ thuật (OA_..., EXP_..., PRJ_...) trong câu trả lời; chỉ dùng tên người/dự án/tổ chức đã được cung cấp."""

    source_ref = _source_ref_vi(source_context)
    expert_to_expert = _is_expert_to_expert(rec_type, source_context)

    if rec_type == "expert":
        if expert_to_expert:
            few_shot = """===== VÍ DỤ MẪU (expert từ chuyên gia) =====
Gợi ý: chuyên gia PGS.TS. Trần Văn B cho chuyên gia TS. Nguyễn Văn A
Dữ liệu kết nối:
1. TS. Nguyễn Văn A → Trí tuệ nhân tạo → PGS.TS. Trần Văn B (độ tin cậy: 65%)
2. Dự án Smart Health → PGS.TS. Trần Văn B (độ tin cậy: 55%)

Output mẫu:
TS. Nguyễn Văn A và PGS.TS. Trần Văn B cùng có nền tảng nghiên cứu mạnh trong Trí tuệ nhân tạo, tạo tiền đề tốt cho hợp tác học thuật. Dữ liệu còn cho thấy PGS.TS. Trần Văn B từng tham gia dự án Smart Health, mở ra khả năng mở rộng mạng lưới đồng nghiệp và đồng tác giả. Nhìn chung, đây là đối tác nghiên cứu đáng cân nhắc vì sự tương đồng chuyên môn và mạng lưới liên quan."""
            task = f"""===== NHIỆM VỤ =====
Gợi ý: {label_vi} **{name}**{source_desc}
...
Đây là gợi ý đối tác/cộng tác viên nghiên cứu (Expert → Expert), KHÔNG phải gợi ý chuyên gia cho dự án.
Hãy phân tích vì sao {label_vi} **{name}** là đối tác tiềm năng cho {source_ref} bằng 3–5 câu, tập trung vào chuyên môn chung, kinh nghiệm và mạng lưới cộng tác."""
            return f"{rules}\n\n{few_shot}\n\n{task}"
...
Output mẫu:
Dự án Smart City IoT đang nhắm đến lĩnh vực IoT, đây cũng chính là mảng chuyên môn thế mạnh của PGS.TS. Trần Văn A. Dữ liệu còn cho thấy vị chuyên gia này có mối quan hệ hợp tác mật thiết với Nhóm nghiên cứu B, mở ra tiềm năng mở rộng mạng lưới nghiên cứu cho dự án. Nhìn chung, sự tương đồng về chuyên môn và sự hỗ trợ từ các đối tác liên quan khiến chuyên gia này trở thành một mảnh ghép hoàn hảo để tư vấn và dẫn dắt nghiên cứu."""
        task = f"""===== NHIỆM VỤ =====
Gợi ý: {label_vi} **{name}**{source_desc}
Độ phù hợp: {score:.0%}{metrics_text}
...
Hãy phân tích vì sao {label_vi} **{name}** phù hợp với {source_ref} bằng 3–5 câu chuyên nghiệp, tập trung vào năng lực chuyên môn, kinh nghiệm, và mạng lưới kết nối."""
        return f"{rules}\n\n{few_shot}\n\n{task}"
...
Output mẫu:
Dự án A thuộc hướng nghiên cứu Thị giác máy tính, hoàn toàn khớp với danh mục ưu tiên tài trợ của quỹ NAFOSTED. Đồng thời, hệ thống ghi nhận PGS.TS. Nguyễn B cũng nằm trong mạng lưới đã từng nhận hỗ trợ từ quỹ, cho thấy quỹ rất quan tâm tới các nhóm nghiên cứu trong lĩnh vực này. Tổng thể, đây là một lựa chọn cực kỳ khả thi để dự án tiếp cận nguồn vốn nhằm thúc đẩy các kết quả nghiên cứu công nghệ lõi."""
        task = f"""===== NHIỆM VỤ =====
Gợi ý: {label_vi} **{name}**{source_desc}
Độ phù hợp: {score:.0%}
...
Hãy phân tích vì sao {label_vi} **{name}** phù hợp với {source_ref} bằng 3–5 câu chuyên nghiệp, nhấn mạnh sự khớp lĩnh vực và các bằng chứng về lịch sử hỗ trợ/tài trợ."""
        return f"{rules}\n\n{few_shot}\n\n{task}"
...
Output mẫu:
Smart Agriculture Platform có điểm giao thoa rõ rệt với dự án hiện tại thông qua chủ đề cốt lõi là IoT Sensors. Hệ thống cũng ghi nhận có nhân sự hoặc nhóm nghiên cứu (Dr. Nguyen) liên quan mật thiết đến cả hai bên, giúp tạo điều kiện thuận lợi để trao đổi kinh nghiệm thực tiễn. Vì vậy, đây là một dự án rất đáng tham khảo nếu bạn muốn học hỏi cách thiết kế hệ thống và vận hành thực tế."""
        task = f"""===== NHIỆM VỤ =====
Gợi ý: {label_vi} **{name}**{source_desc}
...
Hãy phân tích vì sao {label_vi} **{name}** đáng để {source_ref} tham khảo hoặc hợp tác bằng 3–5 câu chuyên nghiệp, tập trung vào điểm tương đồng và giá trị thực tiễn."""
        return f"{rules}\n\n{few_shot}\n\n{task}"
...
Output mẫu:
Bạn đang có chuyên môn sâu trong mảng Thiết bị y tế, và đây cũng chính là lĩnh vực hoạt động chủ lực của Công ty MedTech X. Dữ liệu còn cho thấy nền tảng Thị giác máy tính của bạn hoàn toàn có thể ứng dụng vào bài toán AI hỗ trợ chẩn đoán X-quang mà doanh nghiệp này đang quan tâm. Điều này mở ra cơ hội hợp tác to lớn để đưa các nghiên cứu học thuật vào triển khai thành sản phẩm thực tế."""
    task = f"""===== NHIỆM VỤ =====
Gợi ý: {label_vi} **{name}**{source_desc}
Độ phù hợp: {score:.0%}
...
Hãy phân tích vì sao {label_vi} **{name}** là đối tác tiềm năng cho {source_ref} bằng 3–5 câu chuyên nghiệp, tập trung vào mảng hoạt động chung và cơ hội hợp tác/chuyển giao công nghệ."""
    return f"{rules}\n\n{few_shot}\n\n{task}"
...
class PGPRExplainer:
    ...
    def explain_recommendation(
        ...
        """
        recommendation, source_context, name_registry = _enrich_recommendation_for_xai(
        ...
        if self.use_llm:
            ...
            if llm_text:
                nl_explanation = _sanitize_display_text(llm_text.strip(), name_registry)
            else:
            ...
            nl_explanation = _sanitize_display_text(nl_explanation, name_registry)
    ...
    async def async_explain_recommendation(
        self,
        recommendation: Dict[str, Any],
        session: aiohttp.ClientSession,
        rec_type: str = "funder",
        ...
        if self.use_llm:
            ...
            llm_text = await async_llm_explain_paths_ollama(
                prompt=prompt,
                session=session,
                model=self.ollama_model,
                ollama_url=self.ollama_url,
    ...
    def _generate_natural_language(
        ...
        path_explanations = self._explain_paths(
            reasoning_paths[:3], rec_type, target_name=name, source_context=source_context
        )
...
{llm_text}"""
    ...
    def _build_intro(
        ...
        *,
        ...
        if _is_expert_to_expert(rec_type, source_context):
            src_name = _resolve_display_name(
                str((source_context or {}).get("source_name") or ""),
                str((source_context or {}).get("source_id") or ""),
            )
            if src_name and src_name != "bạn":
                return (
                    f"Chúng tôi gợi ý {label} **{name}** như đối tác cộng tác tiềm năng "
                    f"cho chuyên gia **{src_name}** vì:"
                )
            return f"Chúng tôi gợi ý {label} **{name}** như đối tác cộng tác tiềm năng vì:"
    ...
    def _build_diversity_text(
        ...
        *,
        ...
        if _is_expert_to_expert(rec_type, source_context):
            return (
                f"Tìm thấy **{diversity} cách khác nhau** để kết nối hai chuyên gia này "
                "qua mạng lưới nghiên cứu"
            )
        st = (source_context or {}).get("source_type") or ""
        src_vi = {
            "Project": "dự án",
    ...
    def _explain_paths(
        ...
        for i, path in enumerate(paths, 1):
            ...
            narrative = self._narrate_structured_path(
                path, rec_type, target_name, source_context=source_context
            )
            explanations.append(
    ...
    def _narrate_structured_path(
        ...
        if len(entities) >= 2 and relations:
            ...
            for index in range(usable):
                ...
                left_name = _resolve_display_name(
                    str(left.get("name") or left.get("id") or "thực thể"),
                    str(left.get("id") or ""),
                )
                right_name = _resolve_display_name(
                    str(right.get("name") or right.get("id") or "thực thể"),
                    str(right.get("id") or ""),
                )
            ...
            if segments:
                if _is_expert_to_expert(rec_type, source_context):
                    conclusion = (
                        "Chuỗi dữ liệu này cho thấy hai chuyên gia có chuyên môn "
                        "hoặc mạng lưới nghiên cứu liên quan."
                    )
                else:
                    conclusion = {
        ...
        path_text = path.get("path", "") or path.get("explanation", "")
        return self._narrate_path(
            path_text, rec_type, target_name, source_context=source_context
        )
    ...
    def _narrate_path(
        ...
        if rec_type == "expert":
            return self._narrate_path_expert(
                steps, target_name, source_context=source_context
            )
        if rec_type == "funder":
    ...
    def _narrate_path_expert(
        ...
        """
        expert_to_expert = _is_expert_to_expert("expert", source_context)
        subject_label = "Hai chuyên gia" if expert_to_expert else "Dự án"
        intro_parts = []
        expert_name = None
        for key, entity in steps:
            entity = _resolve_display_name(entity, entity)
            # Gộp các key có ý nghĩa giống nhau lại
            if key in ["belongs_to_field", "focuses_on_topic", "focuses_on"]:
                if expert_to_expert:
                    intro_parts.append(f"cùng tập trung vào mảng/chủ đề {entity}")
                else:
        ...
        if expert_name:
            if expert_to_expert:
                intro = ", ".join(intro_parts) + "." if intro_parts else "Hai chuyên gia có điểm chung về chuyên môn."
                return (
                    f"{intro} {expert_name} có chuyên môn và mạng lưới phù hợp, "
                    "vì vậy đây là đối tác cộng tác tiềm năng."
                )
        ...
        if intro_parts:
            prefix = f"{subject_label} kết nối qua" if not expert_to_expert else "Kết nối qua"
            return prefix + ": " + " → ".join(intro_parts) + "."
