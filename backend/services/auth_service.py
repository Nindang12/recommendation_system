...
class AuthService:
    ...
    def register(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        payload = self._normalize_skill_fields(dict(payload))
    ...
    def update_profile(self, user_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        payload = self._normalize_skill_fields(dict(payload), partial=True)
    ...
    def _normalize_skill_fields(self, payload: Dict[str, Any], *, partial: bool = False) -> Dict[str, Any]:
        if "skills" in payload and (not partial or payload["skills"] is not None):
            payload["skills"] = self.repo.normalize_skill_name_list(payload.get("skills") or [])
        if "custom_skills" in payload and (not partial or payload["custom_skills"] is not None):
            payload["custom_skills"] = self.repo.normalize_skill_name_list(
                payload.get("custom_skills") or [],
                category="custom",
            )
        return payload
