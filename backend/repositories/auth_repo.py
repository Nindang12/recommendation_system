...
from utils.skill_identity_utils import (
...
class AuthRepository:
    ...
    def _skill_taxonomy(self) -> tuple[Dict[str, Dict[str, Any]], Dict[str, str], Dict[str, str]]:
        cached = getattr(self, "_skill_taxonomy_cache", None)
        if cached is not None:
            return cached
        methods = list(self.db.method_techniques.find({}))
        cached = build_skill_taxonomy_maps(methods)
        self._skill_taxonomy_cache = cached
        return cached
    ...
    def normalize_skill_name_list(
        ...
        *,
        ...
        skill_map, by_compact, by_norm_key = self._skill_taxonomy()
    ...
    def _skill_objects(self, user: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for item in list(user.get("skills") or []) + list(user.get("custom_skills") or []):
            if isinstance(item, dict):
                ...
                if not name:
                    continue
                category = "custom" if normalize_skill_key(name) in custom_skill_set else "unknown"
                ...
                normalized = {
                    ...
                    "category": resolved.get("category") or category,
                }
            if not normalized or not normalized.get("name"):
                continue
            key = normalize_skill_key(normalized["name"])
            if key in seen:
                continue
            seen.add(key)
            skills.append(
                {
                    ...
                    "proficiency_level": normalized.get("proficiency_level")
                    or (item.get("proficiency_level") if isinstance(item, dict) else None)
                    or (item.get("level") if isinstance(item, dict) else None)
                    or "intermediate",
                }
            )
        return skills
