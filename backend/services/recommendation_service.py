...
class RecommendationService:
    ...
    def _build_concise_xai_summary(
        ...
        *,
        ...
        source_context = source_context or {}
        from pgpr.pgpr_xai_explainer import (
            _enrich_recommendation_for_xai,
            _is_expert_to_expert,
        )

        recommendation, source_context, _ = _enrich_recommendation_for_xai(
            recommendation, source_context
        )
        source_name = str(
            source_context.get("source_name")
            or source_context.get("source_id")
            or "nguồn hiện tại"
        )
        target_name = str(
        ...
        return (
            (
                f"Hệ thống đề xuất {target_label} {target_name} như đối tác cộng tác tiềm năng "
                f"cho chuyên gia {source_name} vì {reason}."
            )
            if _is_expert_to_expert(target_type, source_context)
            else (
