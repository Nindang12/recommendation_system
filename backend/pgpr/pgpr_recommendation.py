...
class PGPRRecommender:
    ...
    # Keep Cypher/heuristic fallback available for cold-start nodes and cases
    # where the trained policy cannot sample a valid path.
    ...
    def _calculate_expert_score(
        ...
        h_index = self._safe_float(expert_info.get("h_index"))
        citations = self._safe_float(expert_info.get("citations"))
    ...
    def _recommend_experts_heuristic_improved(
        ...
        candidate_pool_limit = max(limit * 4, 20)
        candidates = self.graph_repo.find_all_reachable_experts_batch(
            project_id,
            max_depth=min(4, self.DEFAULT_HARD_LIMIT_DEPTH),
        ...
        for depth in sorted(candidates_by_depth.keys()):
            ...
            for candidate in candidates_by_depth[depth]:
                ...
                paths = self.find_reasoning_paths(
                    ...
                    target_id=expert_id,
                    target_type="Expert",
                    max_length=min(depth + 1, 4, self.DEFAULT_HARD_LIMIT_DEPTH),
                    timeout=3.0,
                )
                ...
                if score >= min_score:
                    recommendations.append({
                        ...
                        "scoring_method": "cypher_fallback",
                        "candidate_sources": candidate.get("candidate_sources") or ["cypher_fallback"],
                        "reasoning_paths": [
