...
except ImportError:  # Offline mode can still load vocab/triples without Neo4j.
    GraphDatabase = None
    ...
    def export_from_neo4j(self, driver=None) -> None:
        ...
        if driver is None:
            if GraphDatabase is None:
                raise ImportError("neo4j package is required to export KG from Neo4j.")
...
def build_kg_from_neo4j(data_dir: str = "pgpr_data", train_emb: bool = True, emb_dim: int = 64) -> KG:
    ...
    if GraphDatabase is None:
        raise ImportError("neo4j package is required to build KG from Neo4j.")
    driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD))
