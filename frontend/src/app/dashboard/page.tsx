...
export default function DashboardPage() {
  ...
  async function runMultiRecommendation() {
    if (!sourceEntity) {
      setRecommendationError("Tài khoản hiện tại chưa có hồ sơ liên kết để làm nguồn gợi ý.");
      return;
    }
    ...
    try {
      ...
      settledResults.forEach((result, index) => {
        ...
        if (result.status === "fulfilled") {
          ...
          nextErrors[targetType] = result.reason instanceof Error ? result.reason.message : "Gọi gợi ý thất bại";
        }
      });
      ...
      setRecommendationError(error instanceof Error ? error.message : "Gọi gợi ý thất bại");
    } finally {
      setIsLoading(false);
    }
  }
                      ...
                      <div className="col-span-2 rounded-xl border border-slate-100 bg-white p-3 shadow-sm">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          {isSourceEmbeddingLoading ? (
                            ...
                            <Badge
                              variant="outline"
                              className={`rounded-md px-2 py-0.5 text-[10px] font-bold ${embeddingBadge(sourceEmbeddingStatus?.embedding_status).className
                                }`}
                            >
                        ...
                        <p className="mt-2 text-[10px] leading-relaxed text-slate-500">
                          {sourceEmbeddingStatus?.embedding_status === "ready"
                            ? "Embedding đã sẵn sàng, hệ thống có thể dùng hybrid embedding khi gợi ý."
                            : "Embedding chưa sẵn sàng hoặc chưa có tín hiệu, hệ thống sẽ ưu tiên Cypher/PGPR fallback."}
                        </p>
                      </div>
                    </div>
                ...
                {multiTargetTypes.map((type) => {
                  ...
                  return (
                    <Button
                      ...
                      className={`justify-between h-10 rounded-xl px-4 text-xs font-bold transition-all duration-300 border-slate-200 ${isSelected
                    ...
                    >
                      ...
                      <Badge variant={isSelected ? "secondary" : "outline"} className={`rounded-md px-2 py-0.5 text-[10px] font-bold border-transparent ${isSelected
                  ...
                  {recommendations.map((item, index) => (
                    <div key={`${item.id}-${index}`} className="rounded-2xl border border-slate-200/60 bg-white p-5 transition-all duration-300 premium-card-hover relative overflow-hidden">
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-2 text-[10px] font-bold text-slate-400">
                        {item.scoring_method ? (
                          <Badge
                            className={`rounded-md px-2 py-0.5 border-transparent text-[10px] ${item.scoring_method === "pgpr_policy" || item.scoring_method === "hybrid_embedding_path"
